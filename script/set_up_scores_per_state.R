...
set_up_scores_per_state <- function(real_candidates) {
  ...
  write_json(ch_scores, "data/ch_scores.json")
  ...
  write_json(cn_scores, "data/cn_scores.json")
  ...
  write_json(lc_scores, "data/lc_scores.json")
  ...
  write_json(mt_scores, "data/mt_scores.json")
  
  # Writing Scores 
  writing_scores = get_overall_lc_score(real_candidates[real_candidates$lc_presence == 1 & real_candidates$writing_score != 0,])
  for (state in states) {
  ...
  write_json(writing_scores, "data/writing_scores.json")
}
